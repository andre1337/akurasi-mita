...
<td colspan="2"><input name="Simpan" type="submit" id="Simpan" value="Simpan" />
        ...
        <a href="?mnu=kcampaign"><input name="Batal" type="button" id="Batal" value="Batal" /></a>
</td></tr>
</table>
</form>
  ...
  <?php  
  $sqlq="select distinct(status) from `$tbcampaign`  where id_komunitas='".$_SESSION["cid"]."'  order by `id_campaign` desc";
...
<?php  
  $sql="select * from `$tbcampaign` where status='$status' and id_komunitas='".$_SESSION["cid"]."' order by `id_campaign` desc";
  $jum=getJum($conn,$sql);
		if($jum > 0){
//--------------------------------------------------------------------------------------------
$batas   = 2;
...
<img src='$YPATH/$gambar' width='140' height='100' /></a></div>";
				...
				<td valign='top'><div align='center'>
<a href='?mnu=kcampaign&pro=ubah&kode=$id_campaign'><img src='ypathicon/u.png' alt='ubah'></a>
<a href='?mnu=kcampaign&pro=hapus&kode=$id_campaign'><img src='ypathicon/h.png' alt='hapus' 
...
if($jmldata>0){
	...
	if($page > 1){
		$prev=$page-1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$prev&mnu=kcampaign'>« Prev</a></span> ";
	}
	else{echo "<span class=disabled>« Prev</span> ";}

	for($i=1;$i<=$jmlhal;$i++)
	if ($i != $page){echo "<a href='$_SERVER[PHP_SELF]?page=$i&mnu=kcampaign'>$i</a> ";}
	else{echo " <span class=current>$i</span> ";}

	if($page < $jmlhal){
		$next=$page+1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$next&mnu=kcampaign'>Next »</a></span>";
	}
...
$simpan=process($conn,$sql);
	if($simpan) {echo "<script>alert('Data $id_campaign berhasil disimpan !');document.location.href='?mnu=kcampaign';</script>";}
		else{echo"<script>alert('Data $id_campaign gagal disimpan...');document.location.href='?mnu=kcampaign';</script>";}
	}
	else{
	...
	$ubah=process($conn,$sql);
		if($ubah) {echo "<script>alert('Data $id_campaign berhasil diubah !');document.location.href='?mnu=kcampaign';</script>";}
		else{echo"<script>alert('Data $id_campaign gagal diubah...');document.location.href='?mnu=kcampaign';</script>";}
	}//else simpan
}
?>
...
$hapus=process($conn,$sql);
	if($hapus) {echo "<script>alert('Data $id_campaign berhasil dihapus !');document.location.href='?mnu=kcampaign';</script>";}
	else{echo"<script>alert('Data $id_campaign gagal dihapus...');document.location.href='?mnu=kcampaign';</script>";}
}
?>
